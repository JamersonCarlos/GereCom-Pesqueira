...
export default function App() {
  ...
  const [authStep, setAuthStep] = useState<'ENTRY' | 'LOGIN' | 'REGISTER'>(
    'ENTRY'
  );
  const [selectedRole, setSelectedRole] = useState<UserRole | null>(null);

  const [allPlannings, setAllPlannings] = useLocalStorage<Planning[]>(
    'gerecom_plannings',
    []
  );
  const [allServices, setAllServices] = useLocalStorage<Service[]>(
    'gerecom_services',
    []
  );
  const [allNotifications, setAllNotifications] = useLocalStorage<
    Notification[]
  >('gerecom_notifications', []);
  const [allShifts, setAllShifts] = useLocalStorage<Shift[]>(
    'gerecom_shifts',
    []
  );
  ...
  const notifications = useMemo(() => {
    ...
    return allNotifications.filter(
      (n) => n.managerId === currentManagerId && n.userId === currentUser.id
    );
  }, [allNotifications, currentUser, currentManagerId]);

  const shifts = useMemo(() => {
    if (!currentUser) return [];
    if (currentUser.role === 'SECRETARY') return allShifts;
  ...
  const teamMembers = useMemo(() => {
    if (!currentUser || !currentManagerId) return [];
    return users.filter(
      (u) => u.managerId === currentManagerId || u.id === currentManagerId
    );
  ...
  const linkedManagers = useMemo(() => {
    ...
    return users.filter(
      (u) => u.role === 'MANAGER' && u.generalManagerId === currentUser.id
    );
  }, [users, currentUser]);
  ...
  const addNotification = (
    ...
    title: string,
    message: string,
    type: 'service' | 'schedule' | 'general' = 'general',
    relatedId?: string
  ) => {
    if (!currentManagerId) return;
  ...
  const addPlanning = (
    newPlanning: Omit<Planning, 'id' | 'createdAt' | 'status' | 'managerId'>
  ) => {
    if (!currentUser || !currentManagerId) return;
    ...
    addNotification(
      currentManagerId,
      'Novo Planejamento',
      `A secretaria ${planning.department} enviou um novo planejamento.`,
      'service',
      planning.id
    );
  };
  ...
  const updatePlanningStatus = (
    ...
    if (planning) {
      addNotification(
        planning.secretaryId,
        'Status Atualizado',
        `Seu planejamento para ${planning.serviceType} foi ${status === 'APPROVED' ? 'aprovado' : 'rejeitado'}.`,
        'service',
        planning.id
      );
    }
  ...
  const updatePlanning = (updatedPlanning: Planning) => {
    setAllPlannings((prev) =>
      prev.map((p) => (p.id === updatedPlanning.id ? updatedPlanning : p))
    );

    // Check if there's an associated service
    const service = allServices.find(
      (s) => s.planningId === updatedPlanning.id
    );
    if (service) {
      ...
      const addedEmployees = newTeamIds.filter(
        (id) => !oldTeamIds.includes(id)
      );
      ...
      prev.map((s) =>
        s.id === id
          ? {
              ...
              completedBy:
                status === 'WAITING_APPROVAL' ? currentUser?.id : s.completedBy,
            }
          : s
      )
    );
    ...
    if (service) {
      ...
      if (planning && employee && currentManagerId) {
        ...
        addNotification(
          ...
          `O serviço solicitado (${planning.serviceType}) para ${planning.date} foi marcado como ${status}.`,
          'service',
          service.id
        );
      }
      ...
      if (planning) {
        ...
        service.teamIds.forEach((tid) => {
          addNotification(
            tid,
            'Serviço Confirmado',
            `O gerente confirmou o status do serviço ${planning.serviceType}.`,
            'service',
            service.id
          );
        });
    ...
    if (service) {
      ...
      if (planning) {
        ...
        service.teamIds.forEach((tid) => {
          addNotification(
            tid,
            'Revisão de Serviço Solicitada',
            `O gerente solicitou a revisão do serviço ${planning.serviceType}. Por favor, verifique os detalhes.`,
            'service',
            service.id
          );
        });
  ...
  const addShift = (
    newShift: Omit<Shift, 'id' | 'createdAt' | 'managerId'>
  ) => {
    if (!currentManagerId) return;
    const shift: Shift = {
      ...newShift,
    ...
    if (shift.notifyEmployees) {
      shift.employeeIds.forEach((empId) => {
        addNotification(
          empId,
          'Escala de Plantão',
          `Você foi escalado para o plantão do dia ${format(parseISO(shift.date), 'dd/MM/yyyy')}.`,
          'schedule',
          shift.id
        );
      });
  ...
  const addInvitation = (
    invite: Omit<Invitation, 'id' | 'createdAt' | 'managerId'>
  ) => {
    if (!currentUser) return;
  ...
  const handleRegister = (
    ...
    if (managerId) {
      ...
      addNotification(
        ...
        `Novo membro adicionado à sua equipe: ${username} – Nível de acesso: ${roleName}.`,
        'general'
      );
    }
  };
  ...
  const addTeamMember = (
    newMember: Omit<User, 'id' | 'createdAt' | 'managerId'>
  ) => {
    if (!currentUser) return;
    const user: User = {
      ...newMember,
      ...
      case 'planning':
        return (
          <PlanningView
            plannings={plannings}
            onAdd={addPlanning}
            onUpdateStatus={updatePlanningStatus}
            user={currentUser}
      ...
      case 'services':
        return (
          <ServicesView
            ...
            onConfirmStatus={confirmServiceStatus}
            onReviewStatus={reviewService}
            user={currentUser}
            users={users}
          />
        );
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-transparent font-sans relative">
        ...
        <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
          ...
          {(currentUser.role === 'MANAGER' ||
            ...
            <NavItem
              ...
              collapsed={!isSidebarOpen && !isMobile}
              count={
                invitations.filter((i) => i.managerId === currentUser.id).length
              }
            />
          )}
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <div
          className={cn(
            'flex-1 overflow-y-auto p-4 md:p-8 bg-transparent',
            isMobile && 'pb-24'
          )}
        >
